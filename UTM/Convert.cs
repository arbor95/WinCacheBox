...
namespace WinCachebox.UTM
{
    class Convert
  {
    ...
    const double dCvtDeg2Rad = Math.PI / 180.0; // 0.0174532925199432957 ... remember: pi radians = 180 deg
    const double dCvtRad2Deg = 180.0 / Math.PI; // 57.2957795130823208767 ...
    const double eccSquared = 0.00669438; // eccentricity (0.081819191 ^ 2) WGS84
    const double dEquatorialRadius = 6378137.0; // WGS84 (note above: varies from 6,356.750 km to 6,378.135 km)
    const double dScaleFactor = 0.9996; // scale factor, used as k0
    #endregion
                           ...
                           ref string sUtmZone)
    {
      ...
      sUtmZone = iUTM_Zone_Num.ToString() + sUtmLetterActual(dLat);
      ...
      ref double dLon)
    {
      ...
      int iZoneNumber = int.Parse(sZoneNum);
    ...
    private string sUtmLetterActual(double Lat)
    {
      ...
      return (cLetterDesignator.ToString());
    }
